...
if levi:
	...
	cobe="#Decoded By Hso : @Y_3_YY  On Bot : @FS5FF_bot \n\n"
	#source=cobe+source+'\n'+cobe
	#source=source.split('        ')
	#source=''.join(source)
...
#	       source=source.replace('# File: AFRIT(0.1)(2).py (Python 3.9)','#decoded by @Y_3_YY')
	       	...
	       	if 'std::bad_cast' in file:
	       			...
	       			for cop in Copyright:
	       				if cop in file:
	       					fs+=1
	       			if fs > 0:
	       				bot.reply_to(reb,'لا يمكن فك اداه صديقي 🗿')
	       				...
	       				bot.reply_to(reb,f'file decoded By @Y_3_YY\n\nLayers `{la}`')
	       				la=0
...
Force_subsck=[]
tok='6634045516:AAF__3ipOGZ7vjJPppcQgTDXRruKEiY5iAg'
bot=TeleBot(tok)
admin=['1376504610']#,'6707063313']
import sys,os
cu=(os.getcwd())
...
def als(om,m):
	...
	if om:
		try:
			error_msg='لا يعمل البوت فقط في هاذه القروب @pyHimler1 \nارسل ملفك في القروب ليتم فك تشفيره'
